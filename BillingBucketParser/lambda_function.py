...
def lambda_handler(event, context):
    ...
    error_bucket = 'dct-billing-errors-xx'
    
    #Download tge CSV file from S3, read the content, decode from bytes to string, and split the content by lines
    ...
    #Define valid product lines and valid countries
    ...
    for row in csv.reader(data[1:], delimiter=','):
        ...
        if product_line not in valid_product_lines:
            error_found = True
            print(f"Error found in {row[0]}: unrecognized product line {product_line}.")
            break
        # Check if the currency is valid. If not, set error flag to true and print an error message.
        if currency not in valid_currencies:
            error_found = True
            print(f"Error found in {row[0]}: unrecognized currency {currency}.")
            break
        # Check if the bill amount is negative. If not, set error flag to true and print an error message.
        # Check if the date is in the correct format ('%Y-m%-%d'). If not, set error flag to true and print an error message.
        try:
            datetime.strptime(date, '%Y-%m-%d')
        except ValueError:
            error_found = True
            print(f"Error found in {row[0]}: incorrect date format{date}.")
            break
    #After checking all rows, if an error is found, copy the CSV file to the error bucket and delete it from the origial bucket
    if error_found:
        copy_source = {
